...
public class KeyboardInput {
    ...
    public static boolean readBoolean() {
        ...
        if (Arrays.stream(TRUE_INPUTS).anyMatch(s -> s.equalsIgnoreCase(input))) {
            ...
            throw new IllegalArgumentException(
                    "Invalid boolean input! Input " + input + " cannot be parsed into boolean value.");
        }
    }
    ...
    private static <T> T repeatInput(int repetition, String failureMessage, Predicate<T> predicate,
            Supplier<T> supplier) {
        T o = supplier.get();
